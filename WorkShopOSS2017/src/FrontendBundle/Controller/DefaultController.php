...
class DefaultController extends Controller
{
    public function indexAction(Request $request){
        $form = $this->createFormBuilder()
            ->add('Links', CollectionType::class, array('entry_type'   => UrlType::class,
                    'prototype' => true,
                    'allow_add' => true,
                    'entry_options'  => array('attr' => array('class' => 'input-sm')),
                )
            )
            ->add('Category', ChoiceType::class, array('choices'  => array('XML' => 'XML',
                'JSON' => 'JSON',
                'XLS' => 'XLS',),
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($data['Category'] == 'XML'){
                if (is_array($links) && $countLinks >= 2) {
                    for ($i = 0; $i < $countLinks; $i++) {
                        XMLParser::createArray(cURL::fetch_url($links[$i]));
                    }
                }else{
